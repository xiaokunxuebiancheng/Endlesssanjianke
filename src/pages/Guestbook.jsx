      ...
      <div className="flex-1 min-w-0">
        ...
        {replyTo && (
          <div className="flex items-center gap-2 text-xs text-white/40 mb-3">
            ...
            回复中
            <button type="button" onClick={() => setReplyTo(null)} className="text-white/60 hover:text-white ml-1">
              <X size={12} />
            </button>
          </div>
        )}
        ...
        <div className="flex items-center justify-between mt-3">
          ...
          <div className="flex items-center gap-3">
            ...
            {success && <span className="text-xs text-green-400">发送成功</span>}
          </div>
        </div>
      </form>
        ...
        <div className="flex items-center justify-center gap-2 mt-10">
          ...
          {Array.from({ length: totalPages }, (_, i) => (
            <button key={i} onClick={() => setPage(i + 1)}
              className={`w-8 h-8 text-xs rounded-lg transition-colors ${page === i + 1 ? 'bg-white/10 text-white' : 'text-white/50 hover:text-white'}`}>
              {i + 1}
            </button>
          ))}
...
function MessageBubble({ msg, isReply, onReply }) {
  return (
    <div className={`liquid-glass rounded-2xl p-4 ${isReply ? '' : ''}`}>
      <div className="flex items-center gap-2 mb-1.5">
        <span className={`font-medium text-white ${isReply ? 'text-xs' : 'text-sm'}`}>
          {msg.guest_name || '匿名'}
        </span>
      ...
      <p className={`text-white/70 leading-relaxed ${isReply ? 'text-xs' : 'text-sm'}`}>
        {msg.content}
      </p>
      {!isReply && (
        <button onClick={() => onReply(msg.id)}
          className="text-[10px] text-white/30 hover:text-white/60 mt-1.5 transition-colors">
          回复
        </button>
      )}
    </div>
